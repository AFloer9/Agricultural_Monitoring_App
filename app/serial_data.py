...
#sqlalchmodels.Base.metadata.drop_all(bind=engine)  #commented out to not interfere with main (same function)
#sqlalchmodels.Base.metadata.create_all(bind=engine)


def insert_test_data():      #FOR TEST
...
#insert_test_data()  #comment out when not testing--this will throw unique constraint error if run more than once in 
					#same session/same persistent DB
					#(because trying to add aother sensor by the same ID)
...
#def readData(port, sensor_list, read_n_data, loc):  #read Arduino data from serial port
